...
(() => {
  ...
  function refreshQuestionsFromStoresFallback() {
    try {
      ...
      if (fromStores && fromStores.length) {
        autoAnswerState.questions = fromStores;
        if (Date.now() - autoAnswerState.lastDebugAt > 5000) {
  ...
  function findCurrentQuestionFromDom() {
    ...
    const questionText = normalizeText(questionTextEl?.textContent);
    if (!questionText) return null;
    return autoAnswerState.questions.find((q) => normalizeText(getQuestionPrompt(q)) === questionText) || null;
  }
  ...
  function answerCurrentQuestion() {
    ...
    if (!socketManager) {
      ...
      if (!answered) {
        if (Date.now() - autoAnswerState.lastDebugAt > 3000) {
          console.log("[Zyrox][AutoAnswer] No socket manager and no DOM match yet.", {
            questionCount: autoAnswerState.questions.length,
          });
          autoAnswerState.lastDebugAt = Date.now();
        }
      }
      ...
      if (typeof globalQuestionsRaw === "string") {
        try {
          ...
          if (Array.isArray(parsed) && parsed.length) {
            autoAnswerState.questions = parsed;
            autoAnswerState.answerDeviceId = device?.id ?? autoAnswerState.answerDeviceId;
          }
        } catch (_) {}
      }
      const currentQuestionId = deepFindValueByKey(device, (key) => key.includes("_currentQuestionId"));
      if (currentQuestionId) autoAnswerState.currentQuestionId = currentQuestionId;
    }
  }
  ...
  function answerCurrentQuestion() {
    ...
    const socketManager = getSocketManager();
    if (!socketManager) {
      if (globalThis.Phaser && autoAnswerState.currentQuestionId && autoAnswerState.answerDeviceId) {
        const question = autoAnswerState.questions.find((q) => q?._id == autoAnswerState.currentQuestionId);
        const answer = pickAnswerPayload(question);
        ...
        if (answer && roomSender) {
          ...
          autoAnswerState.lastAnsweredId = question?._id || null;
  ...
  var autoAnswerState = globalThis.__ZYROX_AUTOANSWER_STATE__ || {
    intervalId: null,
    listenersAttached: false,
    socketManager: null,
    questions: [],
  ...
  globalThis.__ZYROX_AUTOANSWER_STATE__ = autoAnswerState;
